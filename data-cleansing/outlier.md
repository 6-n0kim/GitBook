...
* 날짜 논리 오류: 거래일시가 생년월일보다 빠른 데이터(예: 출생 전 거래)는 오기입 데이터로 판단하여 제거합니다.
* 이상 거래 및 음수 잔액: 단 1건 발생하는 이상 거래유형('송금') 및 음수 잔액('-5,369,320원') 데이터 행을 처리합니다.

> 📌 참고: 이상치는 무조건 삭제하는 것이 아니라, 도메인 지식으로 정상 범위인지 먼저 판단해야 합니다. 예를 들어 마이너스 통장(대출 한도 내 잔액)이라면 음수가 정상일 수 있습니다. 따라서 이 데이터는 금융 도메인 규칙을 확인한 후 삭제 또는 정상 데이터로 유지할지 결정해야 합니다.
