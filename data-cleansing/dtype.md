...
print(df['생년월일'].dropna().astype(str).str.len().value_counts())

# 하이픈(-)이 없는 값 찾아보기 (NaN은 자동으로 False 처리)
mask = ~df['생년월일'].astype(str).str.contains('-', na=False)
print(df.loc[mask & df['생년월일'].notnull(), '생년월일'])
```
...
df['생년월일'] = df['생년월일'].apply(normalize_date)
```
...
short_addr = df[df['주소'].notnull() & (df['주소'].str.len() < 10)]
